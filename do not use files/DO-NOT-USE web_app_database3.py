...
# ============================================================
    ...
    with st.sidebar:
        ...
        alert_limit = 10
        st.caption(f"📊 Alerts: {len(alerts)}/{alert_limit if not user['premium'] else '∞'}")
    ...
    # ---- DASHBOARD ----
        ...
        if not alerts:
            st.info("No alerts set yet! Click ➕ Add Alert to get started.")
        else:
            settings = get_user_settings(username)
            ...
            for idx, alert in enumerate(alerts):
                ...
                # Row container
                with st.container():
                    col1, col2, col3, col4, col5, col6 = st.columns([1, 1, 1, 1, 1.5, 1.2])
                    ...
                    with col6:
                        ...
                        with btn_col1:
                            if st.button("✏️", key=f"edit_{idx}", help="Edit this alert"):
                                st.session_state[f'editing_{alert["id"]}'] = True
                        with btn_col2:
                            if st.button("🗑️", key=f"del_{idx}", help="Delete this alert"):
                                delete_alert(alert['id'])
                                st.rerun()

                # ---- Inline Edit Form ----
                if st.session_state.get(f'editing_{alert["id"]}', False):
                    with st.container():
                        st.markdown(
                            f"<div style='background:#f8f9fa; padding:16px; "
                            f"border-left:4px solid #2E86AB; border-radius:4px; margin:4px 0 12px 0;'>",
                            unsafe_allow_html=True
                        )
                        st.markdown(f"**✏️ Edit Alert — {alert['symbol']}**")
                        ...
                        with ecol1:
                            new_target = st.number_input(
                                "New Target Price",
                                min_value=0.01,
                                value=float(alert['target']),
                                key=f"new_target_{alert['id']}"
                            )
                        with ecol2:
                            new_type = st.selectbox(
                                "Alert When",
                                ["above", "below"],
                                index=0 if alert['type'] == 'above' else 1,
                                key=f"new_type_{alert['id']}"
                            )
                        with ecol3:
                            st.markdown("<br>", unsafe_allow_html=True)
                            save_col, cancel_col = st.columns(2)
                            with save_col:
                                if st.button("💾 Save", key=f"save_{alert['id']}", type="primary"):
                                    supabase.table('alerts').update({
                                        'target': new_target,
                                        'type': new_type
                                    }).eq('id', alert['id']).execute()
                                    ...
                                    st.rerun()
                            with cancel_col:
                                if st.button("✖ Cancel", key=f"cancel_{alert['id']}"):
                                    st.session_state[f'editing_{alert["id"]}'] = False
                                    st.rerun()
