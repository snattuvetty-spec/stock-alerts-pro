...
if current_time - st.session_state.last_refresh > 120:  # 2 minutes
...
with st.expander("⚠️ Important Disclaimer - Please Read"):
    ...
    **Natts Digital Pty Ltd is a technology company, 
...
# Logged in section
    ...
    if st.session_state.current_page == 'upgrade':
        ...
        if not alerts:
            ...
            st.subheader("Manage")
            cols = st.columns(4)
            for idx, alert in enumerate(alert_data):
                col = cols[idx % 4]
                with col:
...
# Footer - appears on ALL pages (outside all if/else blocks - NO indentation)
st.markdown("---")
st.markdown("""
...
## ⚠️ Critical - Indentation:

#The footer must have **ZERO indentation** (at the left edge) because it needs to appear on **ALL pages**, not just inside the dashboard block.
